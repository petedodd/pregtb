...
births$estimate <- as.numeric(births$estimate*1000)/5
births <- births %>% 
  unnest(year) %>% 
  select(-from,-to) %>% 
...
# births_1$iso3 <- countrycode(births$`Region, subregion, country or area *`, 'country.name', 'iso3c')
# # b<- births_2 %>% filter(!country %in% df_ISO$country)
...
# 
# 
# births_2 <- births_1 %>% select(-c("Notes", `Country code`)) %>% left_join(df_ISO, by = "country") 
...
births_1$iso3 <- countrycode(births_1$country, 'country.name', 'iso3c')
births_2017 <- births_1 %>% filter(year==2017) %>% select(country,iso3, age_group, births_best, births_lo, births_hi)
...
pop_f1$iso3 <- countrycode(pop_f1$country, 'country.name', 'iso3c')
pop_f2017 <- pop_f1 %>% filter(year==2017) %>% select(iso3, age_group, year, pop_f)
length(unique(pop_f2017$iso3))
...
num_births <- births_2017 %>% left_join(pop_f2017, by=c("iso3", "age_group")) %>% select(-year, -country)
length(unique(num_births$iso3))
...
new_df_births <- df_3 %>% select(country,iso3, g_whoregion, year, age_group, best, lo, hi) %>% left_join(num_births, by=c("iso3", "age_group")) %>%
