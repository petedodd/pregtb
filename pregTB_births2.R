...
births_2017 <- births_2 %>% filter(year==2017) %>% select(country,iso3, g_whoregion, age_group, births_best, births_lo, births_hi)
...
pop_f2 <- pop_f1 %>% left_join(df_ISO, by = "country") %>% select(country, iso3, age_group, year, pop_f)
