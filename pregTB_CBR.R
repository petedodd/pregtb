...
cbr_wide <- cbr_wide %>% left_join(ISO, by = "country") 
...
pop_f <- pop_f %>% left_join(ISO, by = "country") %>% select(country,iso3, age_group, year, pop_f)
tpop_wide <- tpop_wide %>% left_join(ISO, by = "country")

pop_f2017 <- pop_f %>% filter(year==2017) %>% select(-year)
# pop_all2017 <- pop_all %>% filter(Time==2017) %>% select(-Time) 

num_cbrbirths <- cbr_wide %>% select(-iso2, -iso_numeric) %>% left_join(tpop_wide, by=c("country", "year", "iso3", "g_whoregion")) %>%
  mutate(cbrbirths_lo = Low_CBR*tpop_lo,
         cbrbirths_best = Medium_CBR*tpop_best,
         cbrbirths_hi = High_CBR*tpop_hi)
...
summary_regions2 <- new_df_cbrbirths%>%group_by(g_whoregion)%>%summarise_at(key_parms, funs(sum), na.rm=T)
